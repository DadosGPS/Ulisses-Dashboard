...
""", f"Dashboard_{hoje.strftime('%Y%m%d')}.html")
    ...
    # Contentor condicional — só renderiza a vista selecionada
    _eq_containers = [st.container() if _vista_eq == _EQ_LABELS[i] else None for i in range(5)]
    class _EqSection:
        def __init__(self, idx):
            self._c = _eq_containers[idx]
        def __enter__(self):
            if self._c: self._c.__enter__()
            return self
        def __exit__(self, *a):
            if self._c: self._c.__exit__(*a)
        def __bool__(self): return self._c is not None
    class _EqSections:
        def __getitem__(self, idx): return _EqSection(idx)
    tab_eq = _EqSections()
    ...
    _plan_containers = [st.container() if _vista_plan == _PLAN_LABELS[i] else None for i in range(5)]
    class _PlanSection:
        def __init__(self, idx):
            self._c = _plan_containers[idx]
        def __enter__(self):
            if self._c: self._c.__enter__()
            return self
        def __exit__(self, *a):
        ...
        def __bool__(self): return self._c is not None
    class _PlanSections:
        def __getitem__(self, idx): return _PlanSection(idx)
    tab_plan = _PlanSections()
