...
def scatter_jogadores(df_plot: pd.DataFrame, x_col: str, y_col: str,
    ...
    for grupo in grupos:
        ...
        for _, row in sub.iterrows():
            ...
            for m in ["Carga Interna","PSE Sessão","Distância Total (m)","HSR (m)","Sprint (m)","Vel. Máx (km/h)"]:
...
def renderizar_bloco(bloco_key: str, df_base: pd.DataFrame,
    ...
    METS_GPS = ["Distância Total (m)", "HSR (m)", "Sprint (m)",
                "Acc (n)", "Dcc (n)", "Carga Interna", "PSE Sessão", "Vel. Máx (km/h)"]
...
if seccao == "dashboard":
    ...
    if mc_recente and "Carga Interna" in df.columns:
        ...
        k1.metric("⚡ Carga Interna", f"{ci_media:,.0f} UA" if not pd.isna(ci_media) else "—", delta=delta_ci)
    else:
        k1.metric("⚡ Carga Interna", "—")

    # ACWR
    acwr_status = "🟢" if acwr_media and 0.8 <= acwr_media <= 1.3 else "🟡" if acwr_media else "🔴"
    k2.metric(f"{acwr_status} ACWR Médio", f"{acwr_media:.2f}" if not pd.isna(acwr_media) else "—",
              help="Zona segura: 0.8–1.3")

    # Hooper
    if not pd.isna(hooper_media):
        hooper_ant = df[df["Microciclo (Nr)"]==mc_anterior]["Hooper Index"].mean() if mc_anterior and "Hooper Index" in df.columns else np.nan
        delta_h = f"{((hooper_media-hooper_ant)/hooper_ant*100):+.0f}%" if not pd.isna(hooper_ant) and hooper_ant > 0 else None
        k3.metric("💤 Hooper Index", f"{hooper_media:.1f}/20", delta=delta_h, delta_color="inverse",
                  help="↓ = melhor recuperação")
    else:
        k3.metric("💤 Hooper Index", "—")

    # Alertas count
    k4.metric("🚨 Jogadores em Risco", str(n_risco), help="ACWR>1.5 + Wellness>14 + Vmáx>7dias")
...
""", f"Dashboard_{hoje.strftime('%Y%m%d')}.html")
    ...
    with tab_eq[0]:
        ...
        METS_SCATTER = ["Distância Total (m)", "HSR (m)", "Sprint (m)", "Vel. Máx (km/h)",
    ...
    with tab_eq[1]:
            ...
            if "Posição" not in df_f.columns or df_f["Posição"].isna().all():
                ...
                st.caption("") ; pass  # não parar app dentro de tab
            ...
            if metrica_pos in df_f.columns:
                ...
                METS_SC_POS = ["Distância Total (m)", "HSR (m)", "Sprint (m)", "Vel. Máx (km/h)",
                               "Acc (n)", "Dcc (n)", "Carga Interna", "PSE Sessão"]
                mets_sc_pos = [m for m in METS_SC_POS if m in df_f.columns]
    ...
    with tab_eq[2]:
            ...
            METS_COMP = ["Distância Total (m)", "HSR (m)", "Sprint (m)", "Acc (n)", "Dcc (n)",
                         "Carga Interna", "PSE Sessão", "Vel. Máx (km/h)", "Hooper Index"]
            ...
            if nivel_comp == "Equipa":
                ...
                mets_radar = [m for m in ["Distância Total (m)","HSR (m)","Sprint (m)","Carga Interna","PSE Sessão","Vel. Máx (km/h)"] if m in df.columns]
                if len(mets_radar) >= 3:
                    max_vals = {m: df[m].max() for m in mets_radar}
        ...
        **Data · Jogador · Posição · Tipo Teste · Altura Salto (cm) · Potência Relativa (W/kg) · RSI (m/s) · Assimetria (%) · Fase Excêntrica (s) · Fase Concêntrica (s) · RFD (N/s) · Microciclo (Nr) · Observações**
                ...
                if "Altura Salto (cm)" in df_latest.columns and "RSI (m/s)" in df_latest.columns:
                    ...
                    if not df_sc_nm.empty:
                        ...
                        fig_nm.update_layout(height=520, xaxis_title="Altura Salto CMJ (cm)", yaxis_title="RSI (m/s)",
                            plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font_color="white", margin=dict(t=20))
                        st.plotly_chart(fig_nm, use_container_width=True)

            with tab_nm2:
                mets_rank_nm = [m for m in ["Altura Salto (cm)","Potência Relativa (W/kg)","RSI (m/s)"] if m in df_latest.columns]
                if mets_rank_nm:
                    ...
                    fig_rk_nm.update_layout(height=max(300,len(df_rk_nm)*42), xaxis_title=met_nm,
                        plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font_color="white", margin=dict(t=5))
            ...
            with tab_nm3:
                if "Assimetria (%)" in df_latest.columns and df_latest["Assimetria (%)"].notna().any():
                    ...
                    fig_asy.update_layout(height=340, yaxis_title="Assimetria (%)",
                        plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font_color="white", margin=dict(t=10))
        ...
        # ═══════════════════════════════════════════════════════════════════════════════
            ...
            with tab_nh1:
                ...
                fig_vn.update_layout(barmode="overlay", height=max(300,len(df_vs)*42),
                    ...
                    plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
                    font_color="white", margin=dict(t=5))
        ...
        # ═══════════════════════════════════════════════════════════════════════════════
            ...
            if st.button("🔍 Validar agora", type="primary"):
                ...
                cols_crit = [c for c in ["Data","Jogador","Tipo","Dia MD","PSE Sessão","Distância Total (m)"] if c in df.columns]
    ...
    with tab_plan[2]:
            ...
            **Lógica:** defines a carga semanal planeada como percentagem de um jogo de referência.
            ...
            METS_PVR = [
                "Carga Interna", "Distância Total (m)", "HSR (m)",
                "Sprint (m)", "Acc (n)", "Dcc (n)"
            ]
            mets_pvr_disp = [m for m in METS_PVR if m in df.columns]
