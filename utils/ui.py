...
<p style="color:#888;font-size:0.85em">Gerado automaticamente · {pd.Timestamp.now().strftime("%d/%m/%Y %H:%M")}</p>
{conteudo_html}
</body></html>"""
...
def metric_card(label: str, value: str, delta: str = "", cor: str = "#e63946"):
    st.markdown(
        ...
        f'{"<div style=\'font-size:0.72rem;color:rgba(255,255,255,0.4);margin-top:2px\'>" + delta + "</div>" if delta else ""}'
        f'</div>',
        unsafe_allow_html=True
    )
...
def sem_dados_suficientes(minimo: int = 4, atual: int = 0, metrica: str = "ACWR"):
    st.info(
        ...
        f"são necessárias pelo menos **{minimo} semanas** de dados. "
        f"Actualmente tens **{atual}** sessão(ões) registada(s)."
    )
