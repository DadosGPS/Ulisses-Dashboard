...
def verificar_password():
    try:
        pwd_correta = st.secrets["password"]
    except Exception:
        pwd_correta = "belenenses2025"  # password por defeito se não houver secrets
    ...
    if not st.session_state["autenticado"]:
        ...
        <div style='max-width:400px;margin:80px auto;text-align:center'>
            <img src='https://upload.wikimedia.org/wikipedia/pt/thumb/3/3a/C.F._Os_Belenenses.svg/120px-C.F._Os_Belenenses.svg.png' width=90>
            <h2 style='color:#e63946;margin-top:16px'>Dashboard de Carga</h2>
            <p style='color:#aaa'>Belenenses — Acesso Restrito</p>
        </div>
        """, unsafe_allow_html=True)
        col_pw = st.columns([1,2,1])[1]
        pwd = col_pw.text_input("Password", type="password", key="pwd_input")
        if col_pw.button("Entrar", type="primary", use_container_width=True):
            if pwd == pwd_correta:
                st.session_state["autenticado"] = True
                st.rerun()
            else:
                col_pw.error("Password incorreta")
        st.stop()
