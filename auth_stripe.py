...
✓ Notificações email e WhatsApp
    ...
    if st.button("⬆️ Activar Plano Pro — 29€/mês",
        ...
        if "url" in resultado:
            url = resultado["url"]
            # Abrir Stripe via JavaScript — única forma fiável no Streamlit
            st.markdown(
                f"""
<script>window.open("{url}", "_blank");</script>
<div style="background:#e63946;border-radius:10px;padding:16px;text-align:center;margin:8px 0">
<div style="font-size:0.9rem;font-weight:700;color:white;margin-bottom:8px">
💳 Página de pagamento gerada!</div>
<a href="{url}" target="_blank"
style="display:inline-block;background:white;color:#e63946;
padding:10px 24px;border-radius:6px;font-weight:700;font-size:0.85rem;
text-decoration:none">Abrir página de pagamento →</a>
<div style="font-size:0.68rem;color:rgba(255,255,255,0.6);margin-top:8px">
Se não abriu automaticamente clica no botão acima</div>
</div>""",
